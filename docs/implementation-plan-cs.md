...
### 4.2 Systém barev hráčů a vykreslení kostky
Každý hráč dostane jedinečnou barvu odvozenou z jeho `owner` ID (tj. pozice základny v mapě). Barvy jsou definovány jako CSS custom properties, takže paletu lze rozšiřovat bez zásahu do kódu komponent.

- `PLAYER_COLORS` — pevná seřazená paleta (např. `['#dc2626', '#2563eb', '#16a34a', '#d97706', ...]`) indexovaná pozicí hráče v `state.players`; mapování `owner → barva` se odvozuje jednorázově při startu hry a předává se jako prop nebo React context `playerColors: { [owner]: cssColor }`
- **Zbarvení báze**: `<HexTile>` čte `startingField.owner` z `fieldProperties` a aplikuje barvu daného hráče (světlejší tón) jako výplň hexu
- `<Die value, owner, playerColors, isTop>` — vykreslí jednu plochu kostky (tečky nebo číslice)
  - Barva pozadí se bere z `playerColors[owner]`
  - `isTop` ovládá plnou vs. ztmavennou průhlednost ve věžovém zásobníku
...
- **Barva se nevybírá** — je přiřazena automaticky na základě indexu hráče v `state.players` pomocí palety `PLAYER_COLORS` definované ve Fázi 4.2; znak je překryt na tuto barvu v uživatelském rozhraní
