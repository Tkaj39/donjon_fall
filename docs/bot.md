...
>
> **TODO**: Consider the classic chess triad — **material, space, time** — as a framework for auditing evaluation completeness:
>
> - **Material** (what you have): die count, die face values, combat power. Covered by existing components.
> - **Space** (where you are): focal point control and proximity, edge/hole vulnerability. Covered by existing components.
> - **Tempo / time** (initiative — who forces whom to react): currently **absent** from the evaluation function. In chess, tempo means being a move ahead — threatening something that forces the opponent to defend rather than attack. In Donjon Fall, tempo could manifest as: threatening multiple focal points simultaneously so the opponent cannot cover all of them, or having enough mobility to create two threats in one turn. A bot without a tempo component may fail to recognise and exploit initiative advantages, or fail to defend against them. How to express tempo quantitatively in this game is an open question — one candidate is the number of uncontrolled active focal points reachable by own dice within one turn minus those reachable by the opponent — but this requires further design work.
>
