...
| BUG-013 | game / movement | When a single die passes through a friendly die (combat power condition met), the remainder of its movement should be treated as a jump off a tower: combat power is boosted to the virtual tower's combat value for the next (own dice − enemy dice, min 1) hexes, then reverts to the die's plain face value; remaining movement uses the original face value minus steps already taken (no extra movement granted) | P2 | resolved | `getReachableHexes` BFS and `getPathsToHex` DFS now track boost state `[boostLeft, boostStr]`; passing through a friendly computes a virtual tower boost; `applyMoveAction` uses internal `computePathAttackStrength` over all paths to pick the best attack strength and stores it as `attackStrengthOverride`; UI filter uses new exported `getMoveAttackStrength` to correctly highlight enemy hexes reachable only via a boosted path |
...
| BUG-015 | game / movement | When a die moves off a tower and attacks an enemy, an invalid combat direction can be selected — approach directions that are not reachable via a legal jump path are offered to the player. | P2 | resolved | `getApproachDirections` now accepts a `defenderStr` parameter and filters paths via `computePathAttackStrength` — only directions where the attacker's effective strength strictly exceeds the defender are offered; picker threshold lowered to `dirs.size >= 1` so the single valid direction still appears |
