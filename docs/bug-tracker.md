...
| ID | Component | Description                                                                                                                                                                                                                                                                                                    | Priority | Status | Resolution |
|----|-----------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------|--------|------------|
| BUG-015 | game / movement | When a die moves off a tower and attacks an enemy, an invalid combat direction can be selected — approach directions that are not reachable via a legal jump path are offered to the player. **Fix hint:** `getApproachDirections` uses `getPathsToHex` (single-die rules, full range) for all move-die cases; it needs a jump-aware variant that uses `getJumpReachableHexes` traversal rules and `jumper.value` range when the selected hex is a tower. | P2 | open | — |
